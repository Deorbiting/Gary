...
import { getToolsAsync, getToolRegistryAsync, buildToolDescriptionsFromRegistry } from '../tools/registry.js';
...
export class Agent {
  ...
  /**
   * Create a new Agent instance with tools (including MCP server tools).
   */
  static async create(config: AgentConfig = {}): Promise<Agent> {
    const model = config.model ?? DEFAULT_MODEL;

    // Load MCP tools alongside built-in tools (async)
    const registry = await getToolRegistryAsync(model);
    const tools = registry.map((t) => t.tool);
    const toolDescriptions = buildToolDescriptionsFromRegistry(registry);

    const soulContent = await loadSoulDocument();
    const systemPrompt = buildSystemPrompt(model, soulContent, toolDescriptions);
