/**
 * LangSmith Evaluation Runner for Gary
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Gary agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `gary-marketing-eval-sample-${sampleSize}-${Date.now()}`
      : 'gary-marketing-eval';
    ...
    const experimentName = `gary-eval-${Date.now().toString(36)}`;
